...
export default function HealthBeauty() {
  ...
  const healthProducts = products.filter(
    (product) => product.categoryId === 'health-beauty'
  )
  return (
    <>
    <div className='min-h-screen'>
        ...
        <div className='bg-orange-400 flex py-2 px-5 items-center gap-15'>
            <h1 className='text-4xl font-semibold text-white'> Health & Beauty </h1>
            <Link 
            to={`/product-category/health-beauty`}
            className='text-black font-semibold text-xl border rounded-xl p-1 hover:bg-orange-500 hover:text-white' > 
            View All </Link>
        </div>

        <div className='grid grid-cols-4'> 
    {healthProducts.slice(0,8).map((product)=> (
            <div
            key={product.id} 
